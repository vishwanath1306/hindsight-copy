...
void triggers_agent(HindsightAgentAPI* api) {

	printf("triggers_agent: %p\n", api->triggers.queue.meta);
	...
	while (true) {
		...
		if ((now - last_print) > print_every) {
			...
			printf("Triggers Throughput: %ld\n", tput);
			last_print = now;
			count = 0;
		}

		TriggerBatch batch;
		hindsight_agentapi_get_triggers_nonblocking(api, &batch);
